...
def check_load(load_avg, threshold_high, threshold_low, last_run_time):
    current_time = int(time.time())
    time_diff = current_time - last_run_time

    if load_avg > threshold_high and time_diff >= 300:
        perform_upscale()
        return current_time
    elif load_avg < threshold_low and time_diff >= 300:
        perform_downscale()
        return current_time
    elif time_diff < 300:
...
def main():
    ...
    last_run_time = check_load(load_1min, thresholds['THRESHOLD_HIGH_1MIN'], thresholds['THRESHOLD_LOW_1MIN'], last_run_time)
    last_run_time = check_load(load_5min, thresholds['THRESHOLD_HIGH_5MIN'], thresholds['THRESHOLD_LOW_5MIN'], last_run_time)
    last_run_time = check_load(load_15min, thresholds['THRESHOLD_HIGH_15MIN'], thresholds['THRESHOLD_LOW_15MIN'], last_run_time)
